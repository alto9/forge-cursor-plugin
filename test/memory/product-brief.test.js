...
function brief(overrides = {}, narrative = "") {
  const base = {
    doc: "product.brief",
    schema_version: 1,
    updated: "2026-08-21",
...
describe("parseProductBrief", () => {
  ...
  it("rejects wrong schema_version", () => {
    const md = brief({ schema_version: 99 });
    const result = parseProductBrief(md);
...
`;
  ...
  it("rejects forbidden Changelog heading in body", () => {
    const result = parseProductBrief(brief({}, "## Changelog\n\nstuff\n"));
  ...
  it("warns on weak brief without failing", () => {
    const result = parseProductBrief(
      brief({ product: "", problem: "p", current_focus: "c", goals: ["g"] })
    );
    expect(result.errors).toEqual([]);
    expect(result.warnings.some((w) => w.includes("weak brief"))).toBe(true);
  });
  ...
  it("warns when body exceeds soft max", () => {
    const result = parseProductBrief(brief({}, "x".repeat(BODY_SOFT_MAX + 1)));
    expect(result.errors).toEqual([]);
...
`;
  ...
  it("returns normalized shape", () => {
    const result = parseProductBrief(brief({}, "Some nuance."));
    expect(result.errors).toEqual([]);
...
describe("validateSchemaDoc routing", () => {
  it("errors on unsupported schema_version", () => {
    const md = brief({ schema_version: 2 });
