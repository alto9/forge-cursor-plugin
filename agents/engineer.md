...
`/forge.implement-ticket` does **not** read or write memory (SCM-only auto-Apply). It loads the issue body **and** the newest tech spec issue comment (`<!-- forge-tech-spec -->`). After claim, it **fetches host default and creates a worktree from that SHA** before any code (see `implement-ticket` skill). `engineering/in-flight.md` remains for other events (`respond-to-review`, PM status) when those Docs are in scope.
