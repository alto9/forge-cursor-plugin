...
Same map is encoded in `scripts/validate-memory.js` (`DOC_TEMPLATE_MAP`). `product/brief.md` is seeded from the template and validated via frontmatter schema (`SCHEMA_DOC_MAP`), not heading match.
