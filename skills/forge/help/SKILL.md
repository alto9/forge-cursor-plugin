...
5. Suggest 1–3 next commands from current state (missing memory-repo or forge.json → setup + `/forge.init-project`; coarse backlog → `/forge.backlog-grooming`; Refinement queue → `/forge.refinement`; Ready + `ai-ready` → `/forge.implement-ticket`; PR ready / In Review → `/forge.validate-ticket`; etc.).
