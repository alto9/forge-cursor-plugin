...
3. Summarize: board/SCM wins, memory layout, HITL, parent owns Apply / subagents propose-only. HITL is two phases in one conversation: **Questions** first (host AskQuestion when available, else markdown; letters belong to a named question; `(Recommended)` first; nothing written), then the **apply-set** with Questions `None`. **approve all** / **approve subset** Applies that set; **reject** Applies nothing; freeform on either phase reshapes and pauses again — never Apply a set the user has not seen.
