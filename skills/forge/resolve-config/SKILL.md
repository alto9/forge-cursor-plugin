...
2. Read `memoryRoot/forge.json`. If missing, run **ensure-config** (or stop and tell orchestrator to run `init-project`).
3. Ensure `forge.json.path` equals `submodulePath`.
4. Expose: `host`, github/gitlab identity, optional `statusIds`, optional `release.gates`.
