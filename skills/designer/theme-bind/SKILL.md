...
3. Resolve `figma_url` from the user/orchestrator or existing memory. Ask for the Figma URL for user-facing apps. Defer unbound only via explicit plan Accept (“no Figma file yet”); then propose `design/structure.md` with `structure_status: fail` and gap `"no theme bound"`.
...
8. When event-spawned: return a plan-delta blob. Do **not** Apply, pause with the orchestrator, or mutate SCM.
