...
4. When event-spawned: return a plan-delta blob. For validate-ticket, Proposed memory edits are none. Do **not** Apply, pause with the orchestrator, or mutate SCM.

## Outputs / stop conditions

Plan-delta blob for the parent command (ephemeral plan and/or proposed memory edits). Stop if path/config unresolved (parent should have run resolve-paths).
