...
4. Propose vendor actions in HITL hand-off before mutating unless parent Apply already approved them, or the parent event is auto-apply (`/forge.validate-ticket`). For validate-ticket: post the required combined PASS/FAIL comment (first line `Forge validate-ticket: PASS` or `FAIL`, then QA and Security verdict lines).
