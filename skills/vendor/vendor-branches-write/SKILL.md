...
5. **Memory-repo refuse:** if the target repository is the memory-repo (checkout path `.ai/memory`, or remote URL matching the `.ai/memory` submodule), **STOP**. Never create branches on the memory-repo — memory uses local `commit-memory` on `main` only.
