...
4. Propose vendor actions in HITL hand-off before mutating unless parent Apply already approved them, or the parent event is auto-apply (`/forge.implement-ticket`). This skill is read/poll only — waiting on CI is not a HITL pause.
...
6. No CI configured on the host for this PR/MR → treat as skipped (success for the implement-ticket gate). Failure or cancelled → not complete; Engineer fixes, parent auto-Applies push, and waits again.
