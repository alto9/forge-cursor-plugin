...
if [[ -z "${REPO_PATH}" || -z "${PR_NUM}" ]]; then
    echo "Error: could not parse PR reference '${ARG}'" >&2
    exit 1
fi

gh pr view "${PR_NUM}" --repo "${REPO_PATH}" \
    --json number,title,url,state,mergeStateStatus,reviewDecision,headRefName,baseRefName,author,reviews,comments
