...
1. Enumerate `initiatives/*/initiative.md` with status `hld` or `intake`.
2. For each: check feature file, spec, design (if user_facing), security, blocking OQs.
...
Hand-off with gate table + optional status transitions. Stop if path/config unresolved.
