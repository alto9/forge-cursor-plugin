...
3. Merge subagent proposals into one hand-off. On conflict, Lead wins unless Instructions say otherwise. **Board/SCM wins over memory.** For tech spec merges: PO owns product boundaries, Architect owns structure, Security owns safety.
...
- **Ready gate** — issue id | pass/fail | failing checklist items | proposed label (`ai-ready` | `human-ready`) | tech spec: pass/fail/N/A | missing sections
...
**If `human-ready`:**
Run product checklist (items 1–9); tech spec is N/A — do not spawn Architect/Security for tech spec writing. PO may still use Architect constraint-mapping to inline product-level constraints into the body. On HITL approve: promote to Ready with `human-ready` only — no tech spec comment.
...
4. Ready gate requires merged tech spec complete (mandatory sections; Constitution Check pass; no open clarifications). Design gaps → stay Refinement or spawn design-spike instead of Ready.
...
Only checklist **pass** (items 1–9 + item 10 pass or N/A) → vendor move to statusIds.ready, apply the readiness label (strip the other), + backlog.md # Ready. Failures stay Refinement or Blocked; never promote weak tickets.
...
HITL must include Ready gate table (pass/fail + label + tech spec pass/fail/N/A). Do not claim ready for /forge.implement-ticket unless promoting with `ai-ready` **and** a complete tech spec comment.
